...
void NVL144PowerControl::handleShutdownRequest(Event event)
{
    ...
    if (isSystemPowerOff())
    {
        ...
        {
            lg2::info(
                "Power already off during power cycle. Setting GPIOs for host state OFF, starting power cycle delay timer, and transitioning to PowerState::waitForPowerCycleDelay");
